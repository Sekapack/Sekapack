                ...
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.currencies.update", [$currency->id]) }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group {{ $errors->has('main_currency') ? 'has-error' : '' }}">
                            <div>
                                <input type="checkbox" name="main_currency" id="main_currency" value="1" {{ $currency->main_currency || old('main_currency', 0) === 1 ? 'checked' : '' }} required>
                                <label class="required" for="main_currency" style="font-weight: 400">{{ trans('cruds.currency.fields.main_currency') }}</label>
                            </div>
