        ...
        <form method="POST" action="{{ route("admin.currencies.update", [$currency->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('main_currency') ? 'is-invalid' : '' }}">
                    <input class="form-check-input" type="checkbox" name="main_currency" id="main_currency" value="1" {{ $currency->main_currency || old('main_currency', 0) === 1 ? 'checked' : '' }} required>
                    <label class="required form-check-label" for="main_currency">{{ trans('cruds.currency.fields.main_currency') }}</label>
                </div>
